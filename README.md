...
一个个人博客 / 名片站：整屏滚动的欢迎页动画、随本地时间连续变色的玻璃质感界面，加上一套带邀请码、用户组授权与限流封禁的多用户鉴权系统；功能入口（标题 / 图标 / 内网地址）全部由后台配置、按权限下发。
...
- 点击后「→」渐变为转圈（双向过渡，不是瞬切）。之后分两条路：
  - **进登录页**：XR 放大居中作底（随后淡出），箭头离开按钮向左下移动，一个影子般的
    副箭头在其下方显影；两枚箭头同时右移，把「登录」「访客登录」两个按钮「带出来」，
    各自成为对应按钮的箭头
  - **直进主页**（免密 / 登录成功）：出场页左移出，被点按钮的克隆飞向侧栏「首页」按钮
    并转向（桌面旋成 ↓ / 手机保持 →）后发射淡出，侧栏按钮依次弹出，正文再沿
    **垂直于侧栏**的方向弹出（桌面从左侧横向 / 手机从下方纵向）
...
- 登录窗的「登录 / 注册」是一枚滑动玻璃胶囊（照搬侧栏那套）；面板切换是两条弹簧的合舞：
  **大底框锚定顶部伸缩**（注册向下抻长、登录向上收短），两块正文横向滑移换位，途中两块
  正文之间始终留有空档；提交按钮里的「→」与转圈双向交接
...
- 邀请码注册（必填），argon2id 哈希存储；邀请码可设**可用次数（1-100）**与**有效期（1-365 天）**
- Redis 会话 + HttpOnly Cookie，7 天滑动续期
- 角色区分：`admin` 可访问 `/admin` 管理后台；管理接口需管理员会话 + CSRF 双重校验
- **访客登录**：无需账号，一键建立只读会话（同样的 HttpOnly Cookie）；管理接口对游客一律 403，
  「更多」页会显示访客身份与「退出访客模式」入口
- **用户组与逐功能授权**：管理员在后台建组、勾选组可见的功能入口、挑选成员；
  用户能打开的功能 = 所隶属组授权集合的并集
...
- 所有写接口（含登出）要求 double-submit CSRF（Cookie 与请求头 token 一致）；功能地址仅接受
  `http(s)://`，图标走 56 项白名单；错误响应不含堆栈、SQL 或内部路径
- 除登录/注册、游客入口与管理员写接口外，**不接受任何输入**（无评论、无留言、无搜索）；
  游客入口不接收任何参数，只建立只读会话，同样受滑动窗口限流保护
- 安全测试套件覆盖会话伪造 / CSRF 全矩阵 / 越权矩阵 / XFF 伪造 / 注入载荷 / 并发消耗等攻面
 （`backend/tests/security_{session,http,input}_test.go`）
...
前端自检用 `npm run check`，已接在 `prebuild` 上，所以 `npm run build` 会先跑完 24 项断言。
...
2. 进入 `/admin` → 「邀请码」分区生成邀请码（可设可用次数与有效期），复制 16 位邀请码
3. 退出，在欢迎页点「登录 →」→ 切到「注册」标签，填入邀请码完成注册

不想注册也可以：登录窗里点「访客登录 →」，以只读身份直接进入主页浏览。

后台分五个分区：邀请码、封禁名单（手动封禁 / 解封 / 清除记录）、用户（角色管理）、
用户组（勾选授权功能 + 挑选成员）、功能配置（标题 / 描述 / 图标 / 内网地址，拖拽排序）。
被授权的用户点主页功能卡片时，地址由服务端鉴权后下发；游客的卡片不可点击。
...
| `HTTP_PORT` | `8088` | 宿主机端口（docker-compose 端口映射用） |
...
| POST | `/api/auth/guest` | 访客入口（无参数，建立只读会话） |
